...
def _resolve_pair_type(pair_type: str) -> str:
    normalized = str(pair_type or "").strip()
    if normalized in {"expert_fork", "expert_fork_model", "expert_fork_expert"}:
        return "expert_fork"
    return normalized


def _is_correct(result: Dict[str, Any], ground_truth_label: Optional[str] = None) -> bool:
    ...
    ground_truth = _normalize_label(result.get("ground_truth_label") or ground_truth_label)
...
def compute_subset_accuracies(results: List[Dict[str, Any]], samples: List[BenchmarkSample]) -> Dict[str, Any]:
    ...
    for sample in samples:
        ...
        is_correct = _is_correct(result, sample.label)
    ...
    return {
        ...
        "acc_medium": _safe_div(medium_correct, medium_total),
...
def compute_accuracy_by_pair_type(results: List[dict], samples: List[BenchmarkSample]) -> Dict[str, float]:
    ...
    counters = {
        ...
        "expert_vs_model": {"correct": 0, "total": 0},
        "model_vs_model_same_model": {"correct": 0, "total": 0},
    ...
    for sample in samples:
        ...
        is_correct = _is_correct(result, sample.label)
        resolved_pair_type = _resolve_pair_type(sample.pair_type)

        if resolved_pair_type in counters:
            counters[resolved_pair_type]["total"] += 1
            if is_correct:
                counters[resolved_pair_type]["correct"] += 1

        if resolved_pair_type in {"expert_fork", "expert_vs_model", "model_vs_model_same_model"}:
            counters["expert_fork"]["total"] += 1
            if is_correct:
                counters["expert_fork"]["correct"] += 1
    ...
    return {
        ...
        "acc_expert_vs_model": _safe_div(counters["expert_vs_model"]["correct"], counters["expert_vs_model"]["total"]),
        "acc_model_vs_model_same_model": _safe_div(
            counters["model_vs_model_same_model"]["correct"], counters["model_vs_model_same_model"]["total"]
        ),
